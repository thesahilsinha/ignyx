...
export async function GET() {
  ...
  const { data, error } = await db.from("comment_rules").select("*").order("created_at", { ascending: false });

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ rules: data, limit: client.plan === "growth" ? 20 : 8, plan: client.plan });
}
...
export async function POST(req: NextRequest) {
  ...
  const { count } = await db.from("comment_rules").select("*", { count: "exact", head: true });
  const limit = client.plan === "growth" ? 20 : 8;
  if ((count || 0) >= limit) {
...
export async function DELETE(req: NextRequest) {
  ...
  const db = getClientDbServiceClient(client.supabase_url, client.supabase_service_key);
  const { error } = await db.from("comment_rules").delete().eq("id", id);
